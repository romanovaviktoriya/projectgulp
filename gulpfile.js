...
var gulp = require('gulp'), //основной плагин gulp
    ...
    csso = require('gulp-csso'),    //структурная минимизация CSS
    ...
    var spriteData =
        gulp.src(path.src.sprites) //выберем откуда брать изображения для объединения в спрайт
            .pipe(spritesmith({
                ...
                cssTemplate: 'less.template.mustache', //файл маски
...
gulp.task('cssOwn:build', function () {
    gulp.src(path.src.css) //Выберем наш основной файл стилей
        ...
        .pipe(prefixer({
            browser: ['last 50 version', "> 1%", "ie 8", "ie 7"]
        })) //Добавим вендорные префиксы
        .pipe(csso())   //структурная минимизация CSS
        .pipe(cssmin()) //Сожмем
